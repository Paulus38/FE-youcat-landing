...
const TakeExamPage: React.FC = () => {
  ...
  const participantId = searchParams.get('participant_id') ? Number(searchParams.get('participant_id')) : undefined;
  
  // Calculate progress percentage
  const progress = examData ? ((currentQuestionIndex + 1) / examData.ExamQuestions.length) * 100 : 0;
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      ...
      await examService.submitExam(
        ...
        examData.guest_identifier || examService.getGuestIdentifier()
      );
    ...
    <Container maxWidth="md" sx={{ py: 3 }}>
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Box 
            sx={{ 
              ...
              bgcolor: remainingTime < 300 ? 'error.light' : 'background.default',
              py: 0.5,
              px: 1.5,
              borderRadius: 2
            }}
        ...
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
            Question {currentQuestionIndex + 1} of {examData.ExamQuestions.length}
          </Typography>
          <LinearProgress 
            variant="determinate" 
            value={progress}
      ...
      >
        <Box>
          ...
          <LinearProgress 
            variant="determinate" 
            value={(countAnsweredQuestions() / examData.ExamQuestions.length) * 100}
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {isAllQuestionsAnswered() 
              ...
              : `You've only answered ${countAnsweredQuestions()} out of ${examData.ExamQuestions.length} questions. Are you sure you want to submit your exam?`}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseConfirmSubmit}>
            Cancel
          </Button>
          <Button 
