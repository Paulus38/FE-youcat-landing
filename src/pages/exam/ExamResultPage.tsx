...
const ExamResultPage: React.FC = () => {
  ...
  const transformApiResponse = (apiResponse: ApiResponse): ExamResult => {
    ...
    const questions: Question[] = apiData.ExamQuestions.map((eq) => {
      ...
      const userAnswer = eq.ExamAnswers.find(
        (answer) => answer.UserAnswers && answer.UserAnswers.length > 0
      );
      ...
      return {
        ...
        category: eq.Question?.QuestionCategory
          ? eq.Question?.QuestionCategory?.name
          : apiData.Book?.name,
  ...
  if (error || !examResult) {
    return (
      <Container maxWidth='md' sx={{ py: 8 }}>
        ...
        <Button variant='contained' component={RouterLink} to='/exam/create'>
          {t('takeAnotherQuiz')}
        </Button>
      </Container>
    );
    ...
    <Container maxWidth='md' sx={{ py: 8 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              ...
              <Box>
                ...
                <Typography
                  variant='h6'
                  fontWeight={600}
                  color={`${performanceLevel.color}.main`}
                >
      ...
      >
        ...
        <Typography variant='body2' color='text.secondary' sx={{ mb: 3 }}>
          {t('questionOf')
            .replace('{current}', (reviewingQuestion + 1).toString())
            .replace('{total}', examResult.questions.length.toString())}
        </Typography>
